...
## Computing priority order:
Pre-processing spreadsheets:
...
head(me_matrix)
```
Define which categories to calculate priority lists for:
```{r}
...
```
Generate relative abundance and presence-absence lists
```{r}
me_list <- list()
rel_list <- list()
```
...
## Computing MCS tree:
Reading SMILES keys and converting to SDF
```{r}
...
Compute pairwise MCS and Tanimoto coefficient
```{r}
...
head(d)
```
Generate hierarchical clustering tree (CHECK THAT YOU'VE GOT THE RIGHT NUMBER OF CLUSTERS!!)
```{r}
...
Calculate bootstrap scores for each cluster (CHECK THAT THIS IS THE RIGHT NAME!!)
```{r}
x <- clusterboot(d,
                 B = 1000,
